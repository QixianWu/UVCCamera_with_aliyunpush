...
public class UVCCamera {
    ...
    protected float mCurrentFrameRate = DEFAULT_PREVIEW_MAX_FPS;
...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
    ...
    public Size getPreviewSize() {
        ...
        // 设置当前帧率
        if (result != null) {
            result.setCurrentFrameRate(mCurrentFrameRate);
        }
        return result;
    }
     ...
     */
    public void setPreviewSize(final int width, final int height, final float min_fps, final float max_fps, final int frameFormat, final float bandwidthFactor) {
        if ((width == 0) || (height == 0))
            throw new IllegalArgumentException("invalid preview size");
        if (mNativePtr != 0) {
            final int result = nativeSetPreviewSize(mNativePtr, width, height, (int) min_fps, (int) max_fps, frameFormat, bandwidthFactor);
            if (result != 0)
